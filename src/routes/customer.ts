...
import { customers as mainTable, users, companies } from "../db/schema.js";
import withPagination from "../util/pagination.js";
import { eq, getTableColumns, and } from "drizzle-orm";
import { customerValidation as schema } from "../validation/schema.js";
...
route.get("/:id", async (c) => {
	...
	const result = await db
		...
		.from(mainTable)
		.innerJoin(companies, eq(companies.id, mainTable.companyId))
		.where(and(eq(companies.id, payload.company.id), eq(mainTable.id, id)))
		.limit(1)
		.execute();
